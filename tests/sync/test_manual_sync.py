...
def test_machine_owned_target_naming():
    target = build_machine_owned_target("2026-05")

    assert target.folder_path == ("HabitOS", "2026")
    assert target.document_name == "2026-05 Habit Dashboard"
    assert target.filename == "2026-05 Habit Dashboard.pdf"
    assert target.display_path == "HabitOS/2026/2026-05 Habit Dashboard.pdf"
